...
// 💡 完美對接您 VPS 本地資料夾的圖標 (不再受外部伺服器影響！)
const HKO_WARNING_BASE = '/icons/hko/warnings/';
const HKO_WEATHER_BASE = '/icons/hko/weather/';

const getWarningData = (code, originalName) => {
  switch(code) {
    case 'WRAINA': return { text: '黃色暴雨警告信號', img: HKO_WARNING_BASE + 'wraina.gif', style: 'bg-[#eab308] text-white shadow-sm' };
    case 'WRAINR': return { text: '紅色暴雨警告信號', img: HKO_WARNING_BASE + 'wrainr.gif', style: 'bg-[#cf4747] text-white shadow-sm' };
    case 'WRAINB': return { text: '黑色暴雨警告信號', img: HKO_WARNING_BASE + 'wrainb.gif', style: 'bg-black text-white shadow-sm border border-gray-600' };
    case 'WTS': return { text: '雷暴警告', img: HKO_WARNING_BASE + 'ts.gif', style: 'bg-[#eab308] text-white shadow-sm' };
    case 'WHOT': return { text: '酷熱天氣警告', img: HKO_WARNING_BASE + 'vhot.gif', style: 'bg-[#cf4747] text-white shadow-sm' }; 
    case 'WCOLD': return { text: '寒冷天氣警告', img: HKO_WARNING_BASE + 'cold.gif', style: 'bg-[#3b82f6] text-white shadow-sm' };
    case 'WFIREY': return { text: '黃色火災危險警告', img: HKO_WARNING_BASE + 'firey.gif', style: 'bg-[#eab308] text-white shadow-sm' };
    case 'WFIRER': return { text: '紅色火災危險警告', img: HKO_WARNING_BASE + 'firer.gif', style: 'bg-[#cf4747] text-white shadow-sm' };
    case 'TC1': return { text: '一號戒備信號', img: HKO_WARNING_BASE + 'tc1.gif', style: 'bg-white text-slate-800 shadow-sm border border-gray-200' };
    case 'TC3': return { text: '三號強風信號', img: HKO_WARNING_BASE + 'tc3.gif', style: 'bg-white text-slate-800 shadow-sm border border-gray-200' };
    case 'TC8NE': return { text: '八號東北烈風或暴風信號', img: HKO_WARNING_BASE + 'tc8ne.gif', style: 'bg-white text-slate-800 shadow-sm border border-gray-200' };
    case 'TC8NW': return { text: '八號西北烈風或暴風信號', img: HKO_WARNING_BASE + 'tc8nw.gif', style: 'bg-white text-slate-800 shadow-sm border border-gray-200' };
    case 'TC8SE': return { text: '八號東南烈風或暴風信號', img: HKO_WARNING_BASE + 'tc8se.gif', style: 'bg-white text-slate-800 shadow-sm border border-gray-200' };
    case 'TC8SW': return { text: '八號西南烈風或暴風信號', img: HKO_WARNING_BASE + 'tc8sw.gif', style: 'bg-white text-slate-800 shadow-sm border border-gray-200' };
    case 'TC9': return { text: '九號烈風或暴風風力增強信號', img: HKO_WARNING_BASE + 'tc9.gif', style: 'bg-white text-slate-800 shadow-sm border border-gray-200' };
    case 'TC10': return { text: '十號颶風信號', img: HKO_WARNING_BASE + 'tc10.gif', style: 'bg-white text-slate-800 shadow-sm border border-gray-200' };
    case 'SMS': return { text: '強烈季候風信號', img: HKO_WARNING_BASE + 'sms.gif', style: 'bg-white text-slate-800 shadow-sm border border-gray-200' };
    case 'WL': return { text: '山泥傾瀉警告', img: HKO_WARNING_BASE + 'landslip.gif', style: 'bg-yellow-600 text-white shadow-sm border border-yellow-700' };
    case 'FNTSA': return { text: '新界北部水浸特別報告', img: HKO_WARNING_BASE + 'ntfl.gif', style: 'bg-white text-slate-800 shadow-sm border border-gray-200' };
    case 'FROST': return { text: '霜凍警告', img: HKO_WARNING_BASE + 'frost.gif', style: 'bg-white text-slate-800 shadow-sm border border-gray-200' };
    default: 
...
const WarningBadge = ({ img, text, iconBg = "bg-transparent", className = "w-4 h-4 object-contain" }) => {
  ...
  return (
    <div className={`${iconBg} shrink-0 flex items-center justify-center p-0.5`}>
      <img src={img} alt={text} title={text} className={className} />
    </div>
  );
};
...
function MainApp() {
  ...
  const [nearbyRadius, setNearbyRadius] = useState(() => {
    try { return parseInt(localStorage.getItem('kmb_nearby_radius') || '1200'); } catch { return 1200; }
  });
  ...
  const [standMonitorId, setStandMonitorId] = useState(() => {
    try { return localStorage.getItem('kmb_stand_monitor_id') || 'ALL_FAVORITES'; } catch { return 'ALL_FAVORITES'; }
  });
  ...
  const [mapState, setMapState] = useState({ 
    isOpen: false, loadingStops: false, stop: null, routeInfo: null, routeStops: [], error: null 
  });
  const [mapEngineState, setMapEngineState] = useState({
    loadingMap: false, fetched: false, snappedCoords: []
  });
  ...
  // --- 2. 所有 useMemo 宣告區 (完美防止 ReferenceError) ---
  ...
  const getOrFetchAllKmbStops = async () => {
    try {
      const cached = localStorage.getItem('kmb_all_stops_cache_v40');
      if (cached) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        try { localStorage.setItem('kmb_all_stops_cache_v40', JSON.stringify({ timestamp: Date.now(), stops: miniStops })); } catch (e) {}
        return miniStops;
      }
    } catch (e) {}
    return [];
  ...
  const fetchStopDetailsInBatch = async (stopIds, company = 'kmb') => {
    let cache = {};
    const cacheKey = `kmb_stop_details_cache_v40_${company}`;
  ...
  useEffect(() => {
    try {
      ...
      const isCTB = mapState.routeInfo?.company === 'ctb';
      const pinColor = isCTB ? '#3b82f6' : '#e3342f'; 
      const pinShadow = isCTB ? 'rgba(59,130,246,0.4)' : 'rgba(227,52,47,0.4)';

      const customIcon = window.L.divIcon({
          className: 'custom-bus-pin',
          html: `<div style="position: relative; width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; margin-top: -36px; margin-left: -18px; z-index: 1000;">
                   <svg viewBox="0 0 24 24" fill="${pinColor}" style="position: absolute; inset: 0; width: 100%; height: 100%; filter: drop-shadow(0px 4px 4px ${pinShadow});">
                      <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/>
                   </svg>
                   <div style="position: absolute; width: 10px; height: 10px; background: white; border-radius: 50%; margin-bottom: 6px; z-index: 10;"></div>
                 </div>`,
          iconSize: [36, 36],
          iconAnchor: [0, 0] 
      });
  ...
  useEffect(() => {
    if (!mapState.isOpen) {
       ...
       setMapStopEtas([]);
       setMapEngineState({ loadingMap: false, fetched: false, snappedCoords: [] });
       setMapGpsState('idle');
    }
  }, [mapState.isOpen]);
        ...
        <div className="flex flex-col items-end justify-center shrink-0 min-w-[100px] pointer-events-none">
          ...
          <div className="flex items-center justify-end gap-2 sm:gap-3 mt-1.5 sm:mt-2">
            ...
            {secondaryMins !== null && secondaryMins >= 0 ? (
              <span className={`${secondarySize} font-extrabold whitespace-nowrap ${theme.etaSecondary} leading-none`}>
                {secondaryMins}
              </span>
            ) : (
          ...
          <div className="flex flex-col gap-2.5 px-3 sm:px-0 mb-4 mt-2">
            {validWarnings.map((wData, idx) => (
              <div key={idx} className={`flex items-center justify-center gap-3 w-full py-3 px-4 rounded-xl text-base font-black tracking-widest ${wData.style}`}>
                {wData.img && <img src={wData.img} alt={wData.text} className="w-8 h-8 object-contain rounded-md bg-white p-[2px] shrink-0" />}
                ...
                <div key={gIdx} className={`sm:rounded-2xl overflow-hidden shadow-sm border-b sm:border ${theme.groupCardBg}`}>
                  <div className={`flex justify-between items-center px-5 py-3 sm:py-4 text-base sm:text-lg font-black border-b ${theme.groupHeaderText}`}>
                    ...
                    <div className="flex flex-col items-center justify-center shrink-0 mx-2">
                      <span className="bg-[#e3342f] text-white px-6 py-1.5 rounded-full text-sm sm:text-base tracking-widest shadow-sm">{group.groupName}</span>
            ...
            <div className="absolute inset-0 flex flex-col justify-between p-8 md:p-12">
              ...
              <div className="relative z-10 flex flex-col items-start gap-2 text-white">
                <div className="flex items-center gap-3">
                  {weatherInfo.icon && (
                    <img src={`/icons/hko/weather/pic${weatherInfo.icon}.png`} alt="Weather" className="w-16 h-16 drop-shadow-xl" />
                  )}
                  <div className="flex flex-col">
                ...
                {validWarnings.length > 0 && (
                  <div className="flex flex-col gap-1.5 w-full max-w-sm mt-3">
                    {validWarnings.map((wData, idx) => (
                      <div key={idx} className="flex items-center gap-3 px-3.5 py-2 rounded-xl bg-black/40 backdrop-blur-md border border-white/10 shadow-lg text-sm font-bold text-white animate-pulse">
                        {wData.img && <img src={wData.img} alt={wData.text} className="w-6 h-6 object-contain rounded-[4px] bg-white p-[1px]" />}
        ...
        <div className={`w-[30%] h-full flex flex-col z-10 transition-colors shadow-2xl ${theme.appBg}`}>
          <div className="px-4 pt-4 pb-3 border-b border-gray-500/10 flex flex-col gap-2 shrink-0">
            <div className="relative">
              <select value={standMonitorId} onChange={(e) => setStandMonitorId(e.target.value)} className={`w-full text-xs font-bold py-1.5 px-3 rounded-lg border appearance-none outline-none ${theme.inputBg}`}>
    ...
    <div className={`h-screen flex flex-col font-sans transition-colors duration-300 overflow-hidden ${isDarkMode ? 'kmb-dark' : ''} ${theme.appBg}`}>
      <style>{`
        ...
        .gps-pulse-ring {
          ...
          border-radius: 50%;
          animation: gps-pulse-anim 2s infinite ease-in-out;
        }
        .gps-blue-dot {
          position: relative;
          width: 14px;
      ...
      <header className={`px-3 py-3 flex items-center justify-between border-b shadow-sm z-20 shrink-0 transition-colors ${theme.topBar}`}>
        <div className="flex items-center gap-2 min-w-0">
          <button onClick={() => setIsStandMode(!isStandMode)} className={`p-1.5 rounded-md ${isStandMode ? 'bg-white text-red-600 shadow-md' : 'text-white hover:bg-white/10 border border-transparent'}`}>
            <MonitorSmartphone className="w-5 h-5 sm:w-6 sm:h-6" strokeWidth={2.5} />
          </button>
          <h1 className="text-lg sm:text-xl font-black tracking-widest text-white truncate drop-shadow-sm pr-2">
            實時巴士報站
          </h1>
        </div>
        
        <div className="flex items-center gap-1.5 shrink-0">
          {(weatherInfo.temp !== '--' || validWarnings.length > 0) && (
            <div className="flex items-center gap-1.5 bg-black/20 border border-black/10 px-2 py-1 rounded-full shadow-inner">
              {validWarnings.length > 0 && (
                <div className="flex items-center gap-1 pl-1">
                  {validWarnings.map((wData, idx) => wData.img && (
                    <img key={idx} src={wData.img} alt={wData.text} title={wData.text} className="w-5 h-5 sm:w-6 sm:h-6 object-contain rounded-[3px] bg-white p-[1px]" />
                  ))}
                </div>
              )}
              {validWarnings.length > 0 && weatherInfo.icon && <div className="w-[1px] h-4 bg-white/30 mx-0.5"></div>}
              <div className="flex items-center gap-1 pr-1">
                {weatherInfo.icon && (
                  <img 
                    src={`/icons/hko/weather/pic${weatherInfo.icon}.png`} 
                    alt="weather icon" 
                    className="w-5 h-5 sm:w-6 sm:h-6 object-contain drop-shadow-md"
                  />
                )}
                {weatherInfo.temp !== '--' && (
                  <span className="text-sm font-bold text-white ios-num-fix drop-shadow-sm tracking-wider">{weatherInfo.temp}°C</span>
                )}
              </div>
            </div>
          )}
            ...
            <div className={`px-5 py-3.5 flex items-center justify-between shrink-0 shadow-sm z-10 ${mapState.routeInfo?.company === 'ctb' ? 'bg-[#ffcc00] text-yellow-950 border-b border-yellow-500' : 'bg-[#e3342f] text-white border-b border-red-700'}`}>
              <div className="flex flex-col min-w-0 pr-4">
                <div className="flex items-center gap-2">
                  ...
                  {String(mapState.routeInfo?.serviceType || '1') !== '1' && (
                    <span className="bg-yellow-400 text-yellow-900 px-1.5 py-0.5 rounded text-[10px] font-black shadow-sm shrink-0">特別班次</span>
                  )}
                </div>
            ...
            <div className={`h-[40%] min-h-[220px] shrink-0 relative border-b shadow-inner ${isDarkMode ? 'border-zinc-700 bg-zinc-900' : 'border-gray-300 bg-gray-200'}`}>
              ...
              {!mapEngineState.loadingMap && !mapState.error && (
                <div className="absolute bottom-3 right-3 z-[1000] flex flex-col items-end gap-2">
                  {mapGpsState === 'error' && (
                    <div className="bg-red-500 text-white text-[10px] px-2 py-1 rounded shadow-lg font-bold animate-fade-in">
                      無法取得定位，請檢查權限
                    </div>
                  )}
                  <button 
                    onClick={handleLocateUser}
                    className={`backdrop-blur p-2.5 rounded-xl shadow-lg transition-all flex items-center justify-center border ${isDarkMode ? 'bg-zinc-900/90 text-blue-400 border-zinc-700 hover:bg-zinc-800' : 'bg-white/90 text-blue-600 border-gray-200 hover:bg-gray-100'} ${mapGpsState === 'locating' ? 'scale-95 opacity-80' : 'hover:scale-105'}`}
                    title="取得我的位置"
                  >
                    {mapGpsState === 'locating' ? (
                      <RefreshCw className="w-[22px] h-[22px] animate-spin" />
                    ) : mapGpsState === 'error' ? (
                      <X className="w-[22px] h-[22px] text-red-500" />
                    ) : (
              ...
              {mapState.loadingStops ? (
                ...
                <div className="py-2">
                  {mapState.routeStops.map((stop, idx) => {
                    ...
                    const isCTB = mapState.routeInfo?.company === 'ctb';
                    const isSpecialRoute = String(mapState.routeInfo?.serviceType || '1') !== '1';
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={`py-4 flex-1 border-b ${isLast ? 'border-transparent' : (isDarkMode ? 'border-zinc-800' : 'border-gray-100')}`}>
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <span className={`text-[15px] leading-tight mt-0.5 ${isCurrent ? (isDarkMode ? 'font-black text-white' : 'font-black text-slate-900') : (isDarkMode ? 'font-bold text-zinc-300' : 'font-bold text-slate-700')}`}>
                              {stop.name}
                            </span>
                          </div>
                                  ...
                                  mapStopEtas.map((eta, eIdx) => {
                                     ...
                                     return (
                                       <div key={eIdx} className="flex items-start gap-3 sm:gap-4 w-full py-1">
                                          <div className="flex items-center gap-1.5 mt-0.5 shrink-0 w-[80px]">
                                              <Bus className={`w-3.5 h-3.5 ${isMissed ? 'text-gray-300' : 'text-[#3b82f6]'}`} />
                        ...
                        <div key={loc.id} className={`p-4 rounded-xl border shadow-sm flex flex-col gap-3 relative ${isDarkMode ? 'border-zinc-800 bg-zinc-900' : 'border-gray-200 bg-white'}`}>
                          ...
                          <div className="flex flex-col gap-2">
                            {(loc.routes || []).map((r, rIdx) => (
                              <div key={`${r.route}-${r.dir}-${r.serviceType}-${rIdx}`} className={`flex items-center gap-2 p-2 rounded-lg border ${isDarkMode ? 'bg-zinc-800/50 border-zinc-800' : 'bg-slate-50 border-gray-100'}`}>
                                ...
                                {String(r.serviceType || '1') !== '1' && (
                                   <span className="bg-yellow-400 text-yellow-900 px-1.5 py-0.5 rounded text-[9px] font-black shrink-0 whitespace-nowrap shadow-sm">特別</span>
                                )}

                                <input 
                                  type="text"
                                  value={r.customDest !== undefined ? r.customDest : (r.dest || '')}
                                  onChange={(e) => handleUpdateCustomDest(loc.id, r.route, r.dir, r.serviceType, e.target.value)}
                                ...
                                <button 
                                  onClick={(e) => handleDeleteRouteInLocation(loc.id, r.route, r.dir, r.serviceType, e)} 
              ...
              {searchStep === 4 && selectedStop && (
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <select value={customGroupName} onChange={(e) => setCustomGroupName(e.target.value)} className={`w-full py-2 px-3 rounded-lg border font-bold text-sm outline-none ${theme.inputBg}`}>
